...
- Shows a curated top-ten **Needs review** queue in the Cull sidebar, balancing large opportunities, quick finishes, aging work, recent activity, the largest backlog, and the newest shoot. Each recommendation explains why it appears, the active folder stays visible, and **Show more** reveals every remaining folder; completed folders stay tucked into an expandable group. The queue uses a cancellable, directory-only background pass after the active review is ready; Keep, Reject, Undo, and Organize Trash results update its cached counts without rescanning the library, while its refresh button picks up external Finder changes.
