...
- Shows a newest-first **Needs review** queue in the Cull sidebar, with unreviewed and total CR3 counts for each date folder and completed folders tucked into an expandable group. The queue uses a cancellable, directory-only background pass after the active review is ready; Keep, Reject, Undo, and Organize Trash results update its cached counts without rescanning the library, while its refresh button picks up external Finder changes.
