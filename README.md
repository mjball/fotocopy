...
- Quick-exports selected CR3 frames as JPEGs with `⌘⇧E`. Choose an output folder for each export; Fotocopy uses macOS’s native image converter, leaves RAWs untouched, and never overwrites an existing JPEG. Blue outlines show every frame selected for export, while a thin neutral inset identifies the frame currently in view. Plain click selects one frame, `⇧`-click selects an inclusive range, `⌘`-click toggles individual frames, and `⇧⌘`-click adds a range.
