...
- Quick-exports selected CR3 frames as JPEGs with `⌘⇧E`. Choose an output folder for each export; Fotocopy uses macOS’s native image converter, leaves RAWs untouched, and never overwrites an existing JPEG. Plain click selects one frame; `⌘`-click filmstrip thumbnails to export several at once.
