...
- Provides keyboard-driven review: arrow keys navigate frames and bursts; `⌘[`/`⌘]` move to the previous or next date folder with CR3s; `K`/`X` keep or reject the current frame; `⇧K` keeps the current frame and rejects the rest; `⇧X` rejects the burst. The same actions are available in the native **Cull** menu.
