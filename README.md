...
- Quick-exports selected CR3 frames as JPEGs with `⌘⇧E`. Choose an output folder for each export; Fotocopy uses macOS’s native image converter, leaves RAWs untouched, and never overwrites an existing JPEG. The blue outline identifies the frame currently in view; plain click selects one frame for export, `⇧`-click selects an inclusive range, `⌘`-click toggles individual frames, and `⇧⌘`-click adds a range.
