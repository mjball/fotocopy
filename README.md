...
- Shows a curated top-ten **Needs review** queue in the Cull sidebar, balancing burst-aware high-payoff work, quick finishes, aging folders, recent activity, the largest backlog, and the newest shoot. Each recommendation explains why it appears, the active folder stays visible, and **Show more** reveals every remaining folder; completed folders stay tucked into an expandable group. A cancellable directory-only pass publishes the queue first, then disposable low-priority analysis enriches its ranking one folder at a time without blocking active review. Keep, Reject, Undo, and Organize Trash results update cached state without rescanning the library, while the refresh button picks up external Finder changes.
...
- Displays fast embedded JPEG previews, with full-resolution preview loading when you zoom; pinch and pan directly in the main viewer, or double-click to toggle between fit and 6× magnification. The Frames filmstrip and active detail-crop strip follow the selected frame during keyboard navigation; selecting multiple frames replaces the single-image viewer with a grid until one image is chosen again.
...
- Provides keyboard-driven review: left/right arrows navigate frames and cross to the previous or next date folder only from the outer review edges; up/down traverse review groups in sidebar order (bursts followed by Single frames), keeping the selected group visible; `⌘[`/`⌘]` also move between date folders with CR3s; `K`/`X` keep or reject the current blue selection; `⇧K` keeps the current burst frame and rejects the rest; `⇧X` rejects the burst; and `Z` toggles the main photo between fit and full zoom, centered on the current detail target. The same actions are available in the native **Cull** menu.
- Quick-exports selected CR3 frames as JPEGs with `⌘⇧E`. Choose an output folder for each export; Fotocopy uses macOS’s native image converter, leaves RAWs untouched, and never overwrites an existing JPEG. macOS posts a notification when the export completes, including partial and failed results. A single blue outline identifies every selected frame, with no competing focused-frame border; when several frames are selected, the main viewer shows those frames in a grid. Keep and Reject always act on that same blue selection—one image is simply a selection of one—then advance to the next image. Completing the final burst automatically continues at the first undecided single frame in that folder. Plain click selects one frame, `⇧`-click selects an inclusive range, `⌘`-click toggles individual frames, and `⇧⌘`-click adds a range.
...
- Shows a live, compact whole-library distribution in the Full-layout inspector: total image count and storage plus the unrated, kept, and rejected split. Rejected counts remain in the distribution after Fotocopy moves them to Finder’s Trash and after Finder empties it, while storage always reflects files still in the library. Keep, Reject, and Undo update it immediately; **Rescan** refreshes it after external Finder changes. The inspector also keeps a one-click **Reveal** action for the selected photo in Finder.
